...
import ShowCategoryForm from '@/Pages/Items/Partials/ShowCategoryForm.vue';
 ...
 *
 * @type {Ref<HTMLElement>} searchInput - 商品名で検索するためのinput要素を保持するリアクティブな参照
 * @type {Ref<boolean>} searchItemByNameOrCategory - 商品名またはカテゴリで検索するためのモーダルダイアログを表示するかどうかを保持するリアクティブな参照
 * @type {Ref<boolean>} selectCategory - カテゴリを選択するためのモーダルダイアログを表示するかどうかを保持するリアクティブな参照
 */
...
/**
 * モーダルダイアログを表示する関数。
 * openModal - 商品名、カテゴリで検索するためのモーダルダイアログを表示する
 * openCategoryModal - カテゴリを選択するためのモーダルダイアログを表示する
 */
const openModal = () => {
    searchItemByNameOrCategory.value = true;
    nextTick(() => {
        searchInput.value.focus();
    });
};
const openCategoryModal = () => selectCategory.value = true;

/**
 * モーダルダイアログを非表示にする関数。
 */
const closeModal = () => searchItemByNameOrCategory.value = false;
const closeCategoryModal = () => selectCategory.value = false;
...
const handleCategoryPathUpdate = (path) => {
    categoryFilterKey.value = path[path.length - 1];
};

/**
 * 商品名・カテゴリでフィルタするためのキーワードをクリアする関数。
 */
const clearItemNameFilter = () => itemNameFilterKey.value = '';
const clearCategoryFilter = () => categoryFilterKey.value = '';
...
const filterByCategory = (items) => categoryFilterKey.value ? items.filter(item => item.category_id === categoryFilterKey.value.id) : items;
...
const getFilteredItemsAndCount = () => {
    ...
    // お気に入りによるフィルタ
    const filteredByFavorite = filterByFavorite(filtered);
    // カテゴリによるフィルタが適用された商品の件数を計算
    const filteredByCategory = filterByCategory(filteredByFavorite);
...
/**
 * お気に入りによるフィルタされた商品情報の件数を保持する算出プロパティ。
 *
 * @type {Ref<number>} - お気に入りによるフィルタされた商品情報の件数
 */
const filteredByFavoriteCount = computed(() => {
    const { filteredByFavoriteCount } = getFilteredItemsAndCount();
    return filteredByFavoriteCount;
});
...
/**
 * カテゴリによるフィルタされた商品情報の件数を保持する算出プロパティ。
 *
 * @type {Ref<number>} - カテゴリによるフィルタされた商品情報の件数
 */
const filteredByCategoryCount = computed(() => {
    const { filteredByCategoryCount } = getFilteredItemsAndCount();
    return filteredByCategoryCount;
});
    ...
    <AuthenticatedLayout>
        ...
        <section class="text-light body-font">
            ...
            <div class="container px-5 py-10 mx-auto">
                ...
                <!-- 商品一覧 -->
                <!-- お気に入りがなかった場合に表示 -->
                <div class="text-center mx-auto py-16" v-show="!(filteredByFavoriteCount)">
                            ...
                            <div class="p-6">
                                ...
                                <div class="flex items-center flex-wrap ">
                                    ...
                                    <span class="text-gray-400 mr-3 inline-flex items-center lg:ml-auto md:ml-0 ml-auto leading-none text-sm py-1">
                                        ...
                                        </svg><span class="text-light">{{ item.comment_count }}</span>
                                    </span>
                                    <!-- お気に入り -->
            ...
            <Modal :show="searchItemByNameOrCategory" @close="closeModal">
                <div class="p-6">
                    ...
                    <!-- 商品名のヒット件数 -->
                    <div class="mt-3 flex justify-between" v-show="itemNameFilterKey && filteredByNameCount > 0">
                        <p class="text-dark text-sm">商品名で&nbsp;<span class="text-base">{{ filteredByNameCount }}&nbsp;</span>件、見つかりました</p>
                        <button class="hover:bg-gray-300 p-1 rounded-full" @click="clearItemNameFilter">
                    ...
                    <!-- カテゴリのヒット件数 -->
                    <div class="mt-3 flex justify-between" v-show="categoryFilterKey && filteredByCategoryCount > 0">
                        <a class="text-dark text-sm" @click.prevent="closeModal">カテゴリ「<span class="text-base">&nbsp;{{ categoryFilterKey.name }}&nbsp;</span>」で<span class="text-base">&nbsp;{{ filteredByCategoryCount }}&nbsp;</span>件、見つかりました</a>
                        <button class="hover:bg-gray-300 p-1 rounded-full" @click="clearCategoryFilter">
                            <svg class="flex-shrink-0 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M6 6l12 12" />
                                <path d="M6 18L18 6" />
                            </svg>
                    ...
                    <!-- カテゴリに該当するものがなかった場合に表示 -->
                    <div class="mt-3 flex justify-between" v-show="categoryFilterKey && filteredByCategoryCount === 0">
                        <a class="text-dark text-sm" @click.prevent="closeModal">カテゴリ「<span class="text-base">&nbsp;{{ categoryFilterKey.name }}&nbsp;</span>」に該当する商品は見つかりませんでした</a>
                        <button class="hover:bg-gray-300 p-1 rounded-full" @click="clearCategoryFilter">
                            <svg class="flex-shrink-0 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M6 6l12 12" />
                                <path d="M6 18L18 6" />
                    ...
                    <div class="mt-3">
                        <ul>
                            <li class="text-gray-600 p-2 cursor-pointer hover:bg-gray-100 flex justify-between items-center" @click="openCategoryModal">
                                カテゴリから探す
                                <svg class="flex-shrink-0 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <!-- カテゴリ選択モーダル -->
            <ShowCategoryForm :show="selectCategory" @close="closeCategoryModal" :categories="props.categories" @update:categoryPath="handleCategoryPathUpdate" />
